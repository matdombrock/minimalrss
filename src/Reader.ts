...
class RSSReader {
  ...
  private urls: string[];
  constructor(urls: string[], cacheDuration: number = 30 * 60 * 1000) {
    this.urls = urls;
    this.cacheDuration = cacheDuration;
    this.cacheFile = path.join(process.cwd(), 'rss_cache.json');
  }
  async fetchAll(): Promise<RSSResponse> {
    const updateCache = (response: RSSResponse): void => {
    ...
    const checkCache = (): RSSResponse | null => {
  ...
  private async urlToRSSItem(url: string): Promise<RSSItem[]> {
    const cleanDescription = (dirty: string): string => {
    ...
    // Handle channel.link possibly being an array
    let source = json.rss.channel.link;
